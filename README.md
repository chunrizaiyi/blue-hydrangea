...
项目按阶段建立测试资料，文档与源码一同纳入版本管理。第一阶段完成需求追踪和 60 条测试用例设计；第二阶段完成 AI 模拟接口、SQLite 升级及 Android 模拟器应用流程测试。第三阶段在模拟器及小米 14（Android 16 / HyperOS 3）上执行系统与数据检查，真机使用独立测试包，不覆盖正式应用及私人记录。

2026 年 9 月 26 日电脑端全量回归 **58 项通过**，静态检查 **0 个问题**；真机已验证密钥保护、记录数量边界、多图数据持久化及部分计时操作。17 项交互补测按最终复核证据汇总为 **16 项通过、1 项绘制层级问题未关闭**，失败及复测日志均保留。一次 45 分钟息屏倒计时的原生到点偏差为 **+4 毫秒**，亮屏前读取的持久化状态已完成。该数据不代表声音实际延迟、耗电性能或重复稳定性。60 条需求用例与自动化测试数量口径不同，不按 60 条全部通过计算。
...
- [阶段三系统交互执行报告](docs/testing/06-阶段三系统交互执行报告.md)
- [阶段三小米 14 真机执行报告](docs/testing/07-阶段三小米14真机执行报告.md)
- [全量用例覆盖明细](docs/testing/08-全量用例覆盖明细.md)
- [真机问题与复核记录](docs/testing/09-真机问题与复核记录.md)
- [真机与回归证据索引](docs/testing/evidence/2026-09-26/README.md)
